...
DESCRIPTION
  ...
  Command Paragraph
    ...
    "=head4 *Heading Text*"
        ...
        The text in the rest of this paragraph is the content of the
        heading. For example:
    ...
    "=back"
        ...
        the end of your list, use "=back" to end it. The *indentlevel*
        option to "=over" indicates how far over to indent, generally in ems
        (where one em is the width of an "M" in the document's base font) or
        roughly comparable units; if there is no *indentlevel* option, it
        defaults to four. (And some formatters may just ignore whatever
        *indentlevel* you provide.) In the *stuff* in "=item *stuff...*",
        ...
        *   Don't put "=head*n*" commands inside an "=over" ... "=back"
            region.
        ...
        *   And perhaps most importantly, keep the items consistent: either
            ...
            that do, you should preface the bullet- or number-like items
            with "Z<>". See Z<> below for an example.)
    ...
    "=pod"
        ...
        Pod block starts with *any* command paragraph, so a "=pod" command
    ...
    "=for *formatname* *text...*"
        ...
        text (i.e., the text in "=foo targetname text..."), but with "=begin
    ...
    "=encoding *encodingname*"
        ...
        a "=encoding *encodingname*" command very early in the document so
        ...
        *encodingname*, use a name recognized by the Encode::Supported
        ...
        CP-1252 versus UTF-8 encoding, but they may guess wrong. It's best
        to be explicit if you use anything besides strict ASCII. Examples:
  ...
  Formatting Codes
    ...
    "E<escape>" -- a character escape
        ...
        *   "E<number>"
            ...
            "0x" means that *number* is hex, as in "E<0x201E>". A leading
            "0" means that *number* is octal, as in "E<075>". Otherwise
    ...
    "Z<>" -- a null (zero-effect) formatting code
        ...
        not to be considered to be a bullet or number. For example, without
        the "Z<>", the line
    ...
    a formatting code to provide a different font-type for a snippet of
    code. As with all things in Perl, there is more than one way to do it.
    One way is to simply escape the closing bracket using an "E" code:
  ...
  The Intent
    ...
    over four spaces, and have it print out, er, verbatim. And presumably in
    a monospace font.
    ...
    exist for pod2text, pod2html, pod2man (that's for nroff(1) and
    troff(1)), pod2latex, and pod2fm. Various others are available in CPAN.
  ...
  Embedding Pods in Perl Modules
    ...
    an "__END__" or "__DATA__" cut mark, make sure to put an empty line
    there before the first Pod command.
